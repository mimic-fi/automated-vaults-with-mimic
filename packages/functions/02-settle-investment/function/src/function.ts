...
import { Candidate, CurrentInvestment, SubgraphResponse, Yields } from './functionTypes/types'
...
function findExistingInvestmentTarget(): Address {
  ...
  let largestProtocol = Address.zero()
...
function resolveProtocol(addressHex: string): Address {
  const addressLower = addressHex.toLowerCase()
  if (addressLower == inputs.aUSDC.toString().toLowerCase()) return inputs.aavePool
  if (addressLower == inputs.cUSDC.toString().toLowerCase()) return inputs.cUSDC
  if (addressLower == inputs.morphoVault.toString().toLowerCase()) return inputs.morphoVault
  return Address.zero()
}
...
function depositCompound(builder: EvmCallBuilder, safeBalance: BigInt): void {
  ...
  builder = builder.addCall(inputs.cUSDC, supplyData)
}
