...
async function main() {
  ...
  const topicRedeem = keccak256(
    toUtf8Bytes("RedeemRequest(address,address,uint256,address,uint256)")
  ...
  const cid = "QmVRzvrqygbQF4332sTu9eLHiePFR84Ckap5KFGXQ2Hrao";
  const manifest = await client.tasks.getManifest(cid);
  const params = {
    //get this from /build in the deployed function
    taskCid: cid,
    description: "Settle redeem event triggered function",
    input,
    trigger,
    version: "0.0.9",
    manifest,
...
main().catch((err) => console.error("❌ Error:", JSON.stringify(err.body.content.errors)));
